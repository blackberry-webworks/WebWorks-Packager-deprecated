...
import net.rim.tumbler.json4j.internal.JSON4JPBackReader;
 ...
 * Note that it is slightly more efficient to directly parse with the appropriate object than to use this class to do a
 * generalized parse.
 */
public class JSON {
     ...
     * @param order
     *            Boolean flag indicating if the order of the JSON data should be preserved. This parameter only has an effect if
     *            the stream is JSON Object { ... } formatted data. Note: The provided reader is not closed on completion of read;
     *            that is left to the caller. Note: This is the same as calling parse(reader, order, false);
     * 
     * @return Returns an instance of JSONArtifact (JSONObject, OrderedJSONObject, or JSONArray), corrisponding to if the input
     *         stream was Object or Array notation.
     * 
     ...
     * @param order
     *            Boolean flag indicating if the order of the JSON data should be preserved. This parameter only has an effect if
     *            the stream is JSON Object { ... } formatted data.
     * @param strict
     *            Boolean flag to indicate if the content should be parsed in strict mode or not, meaning comments and unquoted
     *            strings are not allowed. Note: The provided reader is not closed on completion of read; that is left to the
     *            caller.
     * 
     * @return Returns an instance of JSONArtifact (JSONObject, OrderedJSONObject, or JSONArray), corrisponding to if the input
     *         stream was Object or Array notation.
    ...
    public static JSONArtifact parse( Reader reader, boolean order, boolean strict ) throws JSONException, NullPointerException {
        ...
        try {
            if( reader != null ) {
                ...
                /*
                 * if (!StringReader.class.isAssignableFrom(readerClass) && !CharArrayReader.class.isAssignableFrom(readerClass)
                 * && !PushbackReader.class.isAssignableFrom(readerClass) && !BufferedReader.class.isAssignableFrom(readerClass))
                 * { bufferIt = true; }
                 */
                ...
                while( ch != -1 ) {
                    switch( ch ) {
                        case '{':
                            pReader.unread( ch );
                            /*
                             * if (bufferIt) { rdr = new BufferedReader(pReader); }
                             */
    ...
    /**
     * Parse a Reader of JSON text into a JSONArtifact. This call is the same as JSON.parse(reader, false, false). Note that the
     * provided reader is not closed on completion of read; that is left to the caller.
     * 
     * @param reader
     *            The character reader to read the JSON data from.
     * 
    ...
    /**
     * Parse a InputStream of JSON text into a JSONArtifact. Note: The provided InputStream is not closed on completion of read;
    ...
    /**
     * Parse a InputStream of JSON text into a JSONArtifact. Note that the provided InputStream is not closed on completion of
     * read; that is left to the caller.
     * 
     * @param is
     *            The input stream to read from. The content is assumed to be UTF-8 encoded and handled as such.
     * @param order
    ...
    /**
     * Parse an InputStream of JSON text into a JSONArtifact. This call is the same as JSON.parse(is, false, false). Note that the
     * provided InputStream is not closed on completion of read; that is left to the caller.
     * 
     * @param is
     *            The input stream to read from. The content is assumed to be UTF-8 encoded and handled as such.
     * 
