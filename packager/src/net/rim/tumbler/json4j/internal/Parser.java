...
package net.rim.tumbler.json4j.internal;

import java.io.IOException;
import java.io.Reader;

import net.rim.tumbler.json4j.JSONArray;
import net.rim.tumbler.json4j.JSONArtifact;
...
public class Parser {
     ...
     * 
     * @param reader
     *            The Reader to use when reading in the JSON stream/string.
     * 
     * @throws JSONException
     *             Thrown if an error occurs in tokenizing the JSON string.
     */
     ...
     *            The Reader to use when reading in the JSON stream/string.
     * @param strict
     *            Boolean indicating if the parser should parse in strict mode, meaning unqoted strings and comments are not
     *            allowed.
     * 
     * @throws JSONException
     *             Thrown if an error occurs in tokenizing the JSON string.
     ...
     * 
     * @throws JSONException
     *             Thrown if an IO error occurd during parse of the JSON object(s).
     */
     ...
     * 
     * @param ordered
     *            Flag to denote if the parse should contruct a JSON object which maintains serialization order of the attributes.
     * 
     * @throws JSONException
     *             Thrown if an IO error occurd during parse of the JSON object(s).
     */
     ...
     *            Flag to denote if the parse should contruct a JSON object which maintains serialization order of the attributes.
     * @param jObj
     *            The JSONObjetc to fill out from the parsing. If null, create a new one.
     * 
     * @throws JSONException
     *             Thrown if an IO error occurd during parse of the JSON object(s).
     */
     ...
     * 
     * @param jObj
     *            The JSONArray to fill out from the parsing. If null, create a new one.
     * 
     * @throws JSONException
     *             Thrown if an IO error occurd during parse of the JSON object(s).
     */
     ...
     * 
     * @param ordered
     *            Flag to denote if the parse should contruct for all JSON objects encounted, a JSON object which maintains
     *            serialization order of the attributes.
     * @param jObj
     *            The JSONArray to fill out from the parsing. If null, create a new one.
     * 
     * @throws JSONException
     ...
     * 
     * @throws JSONException
     *             Thrown if an IO error occurs during parse, such as a malformed JSON object.
     */
    ...
    public JSONObject parseObject( boolean ordered, JSONObject rootObject ) throws JSONException {
        ...
        try {
            ...
            if( lastToken != Token.TokenBraceL )
                throw new JSONException( "Expecting '{' " + tokenizer.onLineCol() + " instead, obtained token: '" + lastToken
                        + "'" );
            lastToken = tokenizer.next();
    ...
    /**
     * Method to parse out a JSON array from a JSON string Same as calling parseArray(false, null)
     * 
     * @throws JSONException
     *             Thrown if a parse error occurs, such as a malformed JSON array.
     */
     ...
     * 
     * @param ordered
     *            Flag to denote if the parse should contruct JSON objects which maintain serialization order of the attributes
     *            for all JSONOjects in the array. *param array An array instance to populate instead of creating a new one.
     * 
     * @throws JSONException
     *             Thrown if a parse error occurs, such as a malformed JSON array.
     */
     ...
     * 
     * @throws JSONException
     *             Thrown if an IO error (read incomplete token) occurs.
     */
     ...
     * @return The java object type that represents the JSON value.
     * @param ordered
     *            Flag to denote if the parse should contruct JSON objects and arrays which maintain serialization order of the
     *            attributes.
     * 
     * @throws JSONException
     *             Thrown if an IO error (read incomplete token) occurs.
