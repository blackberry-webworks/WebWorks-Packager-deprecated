...
/**
 * Interface class to define a set of generic APIs both JSONObject and JSONArray implement. This is namely so that functions such
 * as write, which are common between the two, can be easily invoked without knowing the object type.
 */
public interface JSONArtifact {
    ...
    /**
     * Write this object to the stream as JSON text in UTF-8 encoding, specifying whether to use verbose (tab-indented) output or
     * not. Note that encoding is always written as UTF-8, as per JSON spec.
     * 
     * @param os
     *            The output stream to write data to.
     * @param verbose
    ...
    /**
     * Writer this object to the writer as JSON text, specifying whether to use verbose (tab-indented) output or not. be used for
     * each indent. This is an alternate indent style to using tabs.
     * 
     * @param writer
     *            The writer which to write the JSON text to.
     * @return The passed in writer.
     ...
     * 
     * @param verbose
     *            Whether or not to write in compressed format. Less than one means no indenting, greater than 8 and it will just
     *            use tab.
     * 
     * @throws JSONException
     *             Thrown on errors during serialization.
